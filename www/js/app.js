...
app.controller('recordDetailController', function ($scope, $window, $ionicModal, $location, $timeout, $interval, $ionicSlideBoxDelegate, $ionicScrollDelegate, $state, $filter) {
  ...
  angular.element(document).ready(function () {
    angular.element(".script-p").on("click", function (e) {
      e.preventDefault();

      angular.element(".viewer").animate({
        scrollTop: angular.element(angular.element(this).attr('id')).offset()
  ...
  $scope.isScrollCheck = false;
  $scope.isScroll = function () {

    $scope.isScrollCheck = true;
    //console.log($scope.isScrollCheck);
  };

  $scope.risScroll = function () {
    $scope.isScrollCheck = false;
  };
...
// sticky nav
  ...
  $scope.scriptClcik = function (event) {
    ...
    wavesurfer.seekTo(times / 360/*초단위*/);
    wavesurfer.play();
    if (play_check == 0) {
      play_check = 1;
      $scope.active = true;
  ...
  $scope.scrapScroll = function (anchor) {
    $location.hash(anchor);
    var handle = $ionicScrollDelegate.$getByHandle('preview-content');
    console.log(anchor);
  ...
  $scope.previewScroll = function (anchor) {
    $location.hash(anchor);
    var handle = $ionicScrollDelegate.$getByHandle('scrap-content');
    console.log(anchor);
    handle.anchorScroll();
  };
